...
export default async function handler(req, res) {
  ...
  try {
    ...
    const profileUserId = fields.profileUserId[0]; 

    // Logga värdena för felsökning
    console.log('fields.userId:', fields.userId);
    console.log('fields.profileUserId:', fields.profileUserId);
    ...
    const personalprofilepicture = await prisma.personalProfilePicture.findFirst({
      where: { userId: userId, userIdOfProfilePicture: profileUserId },
      select: { id: true, profilePictureUrl: true },
    });

    // Delete the old profile picture file if it exists
    if (personalprofilepicture && personalprofilepicture.profilePictureUrl) {
      const oldFilePath = path.join("../loop_data/own_profile_pictures", personalprofilepicture.profilePictureUrl);
    ...
    // await prisma.personalProfilePicture.update({
    //   where: { id: userId, userIdOfProfilePicture: profileUserId },
    //   data: { profilePictureUrl: filename },
    // });
    // Kontrollera om posten existerar och uppdatera eller skapa en ny post

        // Logga värdena för ytterligare felsökning
        console.log('userId:', userId);
        console.log('profileUserId:', profileUserId);


    if (personalprofilepicture) {
      // Update the user's profile picture
      await prisma.personalProfilePicture.update({
        where: { id: personalprofilepicture.id },
        data: { profilePictureUrl: filename },
      });
    } else {
      // Create a new profile picture record
      await prisma.personalProfilePicture.create({
        data: {
          userId: userId,
          userIdOfProfilePicture: profileUserId,
